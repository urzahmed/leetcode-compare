...
export function ChartContainer({ config, children, className, ...props }: ChartContainerProps) {
  ...
  React.useEffect(() => {
    ...
    root.style.setProperty("--chart-1", "215 25% 27%")
    root.style.setProperty("--chart-2", "221 83% 53%")

    // Set color variables for each series
    Object.entries(config).forEach(([key, value], index) => {
      const cssVar = `--color-${key.replace(/\s+/g, "-").toLowerCase()}`
      root.style.setProperty(cssVar, value.color)
    })
    ...
    <div className="rounded-lg border bg-background p-2 shadow-sm">
      <div className="grid grid-cols-2 gap-2">
        ...
        {payload.map((item: any) => {
          const configItem = Object.entries(config).find(([key]) => key === item.dataKey)

          if (!configItem) {
            return null
          }

          const [, { label, color }] = configItem
